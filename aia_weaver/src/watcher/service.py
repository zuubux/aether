...
class FileWatcher:
    ...
    async def watch_loop(self) -> None:
        ...
        valid_paths = [p for p in self.target_dirs if p.exists() and p.is_dir()]
        if not valid_paths:
    ...
    def _map_change_type(self, change: Change) -> str:
        if change == Change.added: return "created"
        if change == Change.modified: return "modified"
        if change == Change.deleted: return "deleted"
        return "unknown"

    def _should_ignore(self, file_path_str: str) -> bool:
        path = Path(file_path_str)
        parts, name_lower = path.parts, path.name.lower()
        ignore_dirs = {".git", "__pycache__", ".venv", "node_modules", "target", ".aws", ".ssh", ".gnupg", ".config"}
        if any(d in parts for d in ignore_dirs): return True
        sensitive_exts = {".pem", ".key", ".p12", ".pfx", ".crt", ".keystore", ".kdbx", ".env", ".asc", ".sig"}
        if path.suffix.lower() in sensitive_exts: return True
        sensitive_names = {"id_rsa", "id_ed25519", "credentials", "shadow", "passwd", "authorized_keys", "known_hosts", ".bash_history", ".zsh_history"}
        if name_lower in sensitive_names: return True
        if path.name.startswith(".") or path.name.endswith((".swp", ".tmp", "~")): return True
        return False
