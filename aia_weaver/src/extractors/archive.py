...
ARCHIVE_ZIP_EXTENSIONS = {".zip", ".whl", ".jar", ".epub"}
ARCHIVE_TAR_EXTENSIONS = {".tar", ".tar.gz", ".tgz", ".tar.bz2", ".tar.xz"}
...
def get_archive_type(path: Path | str) -> str:
    ...
    name = Path(path).name.lower()
    if any(name.endswith(ext) for ext in ('.tar.gz', '.tgz', '.tar.bz2', '.tar.xz', '.tar')):
        return "TAR"
    ext = Path(path).suffix.lower()
